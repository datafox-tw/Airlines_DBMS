...
===
前兩個版本：增加了錯誤handling, 防呆機制（error page, 避免輸入非數字） 一些介紹 讓他好用一點

hmm是時候來個v1.0了
老實說我對於現在的資料庫設計和dbms核心內容是滿意的 只是作為一個面向客戶或航空公司的系統來說難用的不行
但我現在最想做的事，是：在2024與2025我大量使用SQLAlchemy 或者是prisma的工具做偏向OOP的物件導向查詢系統 我希望在這個分支中，先做到
1. 將目前的code用SQLAlchemy重構
2. 增加一個dashboard，可以看現在狀態（舉例來說離現在最接近的幾筆） 